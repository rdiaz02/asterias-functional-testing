...
def verify_Limma(data_name, test, other_covars = None, tol = 0.02):
    ...
    if (test == 't_limma'):
        ...
        if other_covars:
            oocr = other_covars.split('.txt')[0] ## root name of covar file
    ...
    if not (test == 'Anova_limma'):
        ...
        if other_covars:
            ...
            out_comparison = rpy.r('comparePomeloLimma(cl0cl1, t1.' + oocr + ')')
            if (out_comparison == 'ERROR: test failed'):
                raise AsteriasAssertionError
            print '\n\n\n\n---- Contrasts comparisons: cl0 vs cl2'
            out_comparison = rpy.r('comparePomeloLimma(cl0cl2, t2.' + oocr + ')')
            if (out_comparison == 'ERROR: test failed'):
                raise AsteriasAssertionError
            print '\n\n\n\n---- Contrasts comparisons: cl1 vs cl2'
            out_comparison = rpy.r('comparePomeloLimma(cl1cl2, t3.' + oocr + ')')
            if (out_comparison == 'ERROR: test failed'):
                raise AsteriasAssertionError
